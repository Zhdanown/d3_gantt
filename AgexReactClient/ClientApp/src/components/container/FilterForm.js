...
function FilterForm({...props}) {
  ...
  crops = [...new Set(crops.map(x => JSON.stringify(x)))].map(x => JSON.parse(x))

  // syncronize checkboxes state with applied filter 
  crops.map(crop => {
    if (props.filteredCrops.find(x => x.id === crop.id))
    crop.checked = false;
  ...
  farms = [...new Set(farms.map(x => JSON.stringify(x)))].map(x => JSON.parse(x))

  // syncronize checkboxes state with applied filter 
  farms.map(farm => {
    if (props.filteredFarms.find(x => x.id === farm.id))
    farm.checked = false;
  ...
  */
  ...
  return (
    <Modal name="addPeriod" onClose={() => history.push("/")}>
      <div className="modal-content">
          <div className="row">
            <div className="col S6">
              <CheckboxList list={farmList} onListChange={newList => setFarmList(newList)}/>
              {/* <Checkbox item={{name: "Выбрать все"}} onInputChange={onAllFarmsChange}/>
              <div className="divider"></div>
              {farmList.map(farm => 
                <Checkbox key={farm.id} item={farm} onInputChange={onFarmsChange}/>
              )} */}
            </div>

            <div className="col s6">
              <CheckboxList list={cropList} onListChange={newList => setCropList(newList)}/>
              {/* {cropList.map(crop => 
                <Checkbox key={crop.id} item={crop} onInputChange={onCropsChange}/>
              )} */}
            </div>
          </div>
...
export default connect(mapStateToProps, { filterCrops })(FilterForm)
